...
void KdTree::range_search(const CoordPoint& min_point, const CoordPoint& max_point, kdtree_node* node, KdNodeVector* result) {
    ...
    // Check if the current node's bounding box overlaps with the search rectangle
    if (bounds_overlap_rectangle(min_point, max_point, node)) {
        // If it overlaps, check if the node itself is within the rectangle
        if (node_within_rectangle(min_point, max_point, node->point)) {
            result->push_back(allnodes[node->dataindex]);
        }
        // Recursively search the left and right subtrees
        range_search(min_point, max_point, node->loson, result);
        range_search(min_point, max_point, node->hison, result);
    } else {
        // If it doesn't overlap, skip this subtree
        return;
    }
}
...
bool KdTree::bounds_overlap_rectangle(const CoordPoint& min_point, const CoordPoint& max_point, kdtree_node* node) {
    for (size_t i = 0; i < dimension; i++) {
        if (node->upbound[i] < min_point[i] || node->lobound[i] > max_point[i]) {
            // If any dimension of the node's bounding box is completely outside the search rectangle, there's no overlap
            return false;
        }
    }
...
bool KdTree::node_within_rectangle(const CoordPoint& min_point, const CoordPoint& max_point, const CoordPoint& point) {
    for (size_t i = 0; i < dimension; i++) {
        if (point[i] < min_point[i] || point[i] > max_point[i]) {
            // If any dimension of the point is outside the rectangle, it's not within the rectangle
            return false;
        }
    }
    // Otherwise, the point is within the rectangle
    return true;
}
